...
InstructionResult block00(gbRom* rom, uint8_t* ram, Registers* reg, uint8_t opcode) {
...
InstructionResult block01(gbRom* rom, uint8_t* ram, Registers* reg, uint8_t opcode) {
...
InstructionResult block10(gbRom* rom, uint8_t* ram, Registers* reg, uint8_t opcode) {
...
InstructionResult prefixCB(gbRom* rom, uint8_t* ram, Registers* reg, uint8_t opcode) {
...
InstructionResult block11(gbRom* rom, uint8_t* ram, Registers* reg, uint8_t opcode) {
    ...
    case 0b11001011: // All prefixed 0xCB opcodes
        instruction_result = prefixCB(rom, ram, reg, *(&((*reg).PC)+1));
...
InstructionResult run_instruction(gbRom* rom, uint8_t* ram, Registers* reg) {
    ...
    {
    case 0:
        instruction_result = block00(rom, ram, reg, opcode);
        break;
    case 1:
        instruction_result = block01(rom, ram, reg, opcode);
        break;
    case 2:
        instruction_result = block10(rom, ram, reg, opcode);
        break;
    case 3:
        instruction_result = block11(rom, ram, reg, opcode);
        break;
    }
    return instruction_result;
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i=0; i<500; i++) {
        ...
        if ((halt_state == 0 || halt_state == 3) && !machine_timeout) {
            instruction_result = run_instruction(&rom, ram, &reg);
